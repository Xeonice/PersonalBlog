...
import { generateSectionSEO, generateStructuredData, SEOData } from '../utils/seo';
...
const IndexPage: React.FC<IndexPageProps> = ({ defaultSection, seo, structuredData }) => {
  ...
  React.useEffect(() => {
    ...
    const initializeSection = () => {
      const navIndex = config.navigation.findIndex(item => item.id === defaultSection);
      if (navIndex !== -1) {
  ...
  React.useEffect(() => {
    ...
    const handleRouteChange = (url: string) => {
      ...
      const navIndex = config.navigation.findIndex(item => item.id === sectionId);
      if (navIndex !== -1) {
        setCurrentSectionIndex(navIndex);
      }
    };
  ...
  const handleNavigationClick = (navId: string) => {
    const navIndex = config?.navigation?.findIndex(item => item.id === navId) ?? 0;
    setCurrentSectionIndex(navIndex);
  ...
  const allSections = React.useMemo(() => {
    ...
    return config.navigation.map(navItem => {
            ...
            <div className={indexStyle.content}>
              {isGalgameMode && allSections.length > 0 ? (
                ...
                config?.navigation?.map((navItem) => {
                  const sectionData = config.sections[navItem.id];
                  if (!sectionData || navItem.id !== currentActiveId) return null;
