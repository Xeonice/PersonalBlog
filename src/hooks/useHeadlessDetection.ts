...
const detectHeadless = (): boolean => {
  ...
  const headlessIndicators = [
    ...
    // 缺少某些浏览器特有的属性
    () => !(window as WindowWithChrome).chrome && userAgent.includes('chrome'),
...
export function useHeadlessDetection(): boolean {
  // 使用惰性初始化避免在 useEffect 中调用 setState
  const [isHeadless] = useState(() => detectHeadless());
